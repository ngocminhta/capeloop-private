...
It separates three execution modes that must not be conflated:

| Mode | Network or key | What it produces |
| --- | --- | --- |
| Offline simulation | No | Synthetic users, choices, profiles, metrics, and request packets generated by repository code |
...
The simulator does not ask Codex or another model to invent synthetic users or
responses. An external model is called only by a live provider command or by a
run containing an `llm_*` updater in `openai` or `openrouter` mode. Every live
path requires `--execute-live`.
...
| OpenRouter writers and selected Claude/Gemini decoders | `OPENROUTER_API_KEY` |
...
| --- | --- | --- |
| Response only | Prior, selected option/attributes, observation | Unselected options, presentation, provenance |
| Full context | Response-only fields plus displayed options, order, default, suggestion, wording, question | Policy provenance and hidden audit labels |
| Provenance aware | Full context plus structured policy provenance | Latent preference and susceptibility |

No model view receives latent truth, choice-randomness keys, or hidden
condition/CRN identifiers.
