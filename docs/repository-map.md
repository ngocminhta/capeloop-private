...
| `cli.py` | Command parsing, run dispatch, replay/live LLM helpers, control/decoder/human analysis, correction-debt diagnostics, schema export, and artifact freeze/verify |
| `config.py` | Strict TOML sections, enumerations, defaults, and per-experiment contracts |
| `control_study.py` | Versioned Experiment A six-control stimuli, registered randomization, diagnostic reference/baseline executors, provider exchange, and bound scoring |
| `h7_control_review.py` | Verified-run direct-statement planning, strict OpenAI/OpenRouter evidence binding, `VolunteeredPreferenceUpdate` conversion, immutable H7 recomputation, and exact review verification |
...
| `llm_outcomes.py` | Shared parsed-model outcome validation and belief conversion |
| `gates.py` | Result-free machine-readable stage-gate diagnostics |
| `power.py` | Experiment A paired power, bounded complete-user Experiment B three-way-interaction power, Monte Carlo uncertainty, and multiple-testing helpers |
...
| `provider_attempts.py` | Shared fsynced physical-attempt ledger, conservative retry accounting, crash reconciliation, and manual-review resume boundary |
...
| `experiment_c_review.py` | Blinded native-state packet export, explicit official-collection or reviewed-generic two-family admission, development-only calibration, reranking/ESR/Gate 5, and locked atomic review publication/verification |
| `experiment_c_robustness.py` | Bounded offline verification and immutable exact-agreement/disagreement review across compatible distinct-seed Experiment C ranking runs |
| `robustness_review.py` | Strict paired sensitivity/Experiment A admission, exact live-model evidence binding, held-out paraphrase recomputation, and immutable six-clause Gate 6 review |
...
| `human_comparison.py` | Strict H8 model-evidence exchange, Experiment A conversion, pair-complete cluster estimand, and independent participant/test-user bootstrap |
| `correction_debt.py` | Stage-gated paired correction/recovery protocol and transparent reference adapter |
| `gate_review.py` | Strict recorded-action/source-review contracts, locked first-party decoder admission, verified Experiment B evidence binding, recomputed Gate 4, and durable staged atomic review publication/verification |
...
| `closed_loop_inference.py` | Complete-user Experiment B contrasts, clustered intervals, and interaction decomposition |
...
| `hypothesis_estimands.py` | Frozen complete-user H1/H2 directional/proximity estimands and H7 mitigation/valid-learning estimands |
...
| `sensitivity_llm_openrouter_smoke.toml` | One-point, two-domain OpenRouter LLM sensitivity transport smoke |
...
| `confirmatory_ci.toml` | Synthetic multi-stratum Experiment A fixture used only to execute the R analysis contract in CI |
...
| `test_closed_loop_inference.py` | Complete-user closed-loop inferential contrasts, interaction decomposition, and clustering |
...
| `test_experiment_b_power.py` | Complete-user three-way contrast, incomplete-block exclusion, bounded deterministic power, Monte Carlo uncertainty, and claim boundary |
| `test_control_study.py` | Content-bound six-control construction, reference/no-update diagnostics, provider exchange, and fail-closed scoring |
| `test_h7_control_review.py` | Exhaustive H7 direct-statement planning, strict provider binding, no-imputation conversion, immutable review, CLI, and tamper verification |
| `test_hypothesis_estimands.py` | Frozen H1/H2/H7 formulas, pairing, cluster uncertainty, missing-data semantics, and runner artifacts |
| `test_human_comparison.py` | H8 scale/cluster estimand, strict evidence bindings, Experiment A conversion, schema parity, and atomic compare CLI |
| `test_sensitivity.py` | Independent grid axes, model-family construction, phase classification/boundaries, retry-aware LLM preflight, and fake-provider artifact retention |
...
| `test_experiment_c_external_review.py` | C decoder-packet bindings, two-family rescore, source-safe locked atomic publication, input-race/failure cleanup, schemas, and tamper verification |
| `test_experiment_c_robustness.py` | Real-run admission, stable/unstable exact cross-seed summaries, duplicate-seed/config rejection, CLI, immutability, and tamper detection |
| `test_robustness_review.py` | Gate 6 pair admission, scientific-config matching, paraphrase/model binding, CLI, source re-verification, and tamper/overwrite rejection |
...
| `test_openai_provider.py` | Provider request bodies, strict parsing, physical retry budgets, durable crash windows, audit repair, tamper rejection, and resumability with no live network |
| `test_openrouter_provider.py` | OpenRouter model/provider constraints, strict response parsing, physical-attempt accounting, and static execution locking |
...
| `test_gate_review.py` | Gate 4 action/source contracts, fail-closed external-evidence import, shared/exclusive locks, source/input race rejection, staged-write cleanup, symlink guards, source-run non-mutation, and review checksums |
...
llm-provider-transport-attempt.schema.json
...
experiment-c-decoder-codebook.schema.json
experiment-c-external-score.schema.json
...
h7-volunteered-evidence.schema.json
...
| `docs/experiment-a-controls.md` | Six positive/negative controls, diagnostic executors, provider exchange, and evidence boundary |
| `docs/h7-volunteered-controls.md` | Direct-statement case construction, OpenAI/OpenRouter collection, provider bindings, derived H7 review, and no-imputation boundary |
| `docs/experiment-c-external-decoder.md` | Exact two-family external rescoring and immutable Experiment C review |
| `docs/experiment-c-robustness.md` | Cross-seed ranking/Gate 5/ESR agreement review |
| `docs/gate6-cross-run-review.md` | Strict cross-family declaration, paired-source requirements, six-clause aggregation, and claim boundary |
...
| `docs/hypothesis-estimands.md` | Frozen H1/H2/H7 estimands and incomplete-evidence semantics |
...
- `workflows/ci.yml` — Python 3.11–3.14 doctor/unittest checks plus a separate
  locked R 4.6.1 synthetic confirmatory-harness execution; all third-party
...
CI restores only the frozen optional R dependency graph; it does not read
credentials or call an LLM provider.
